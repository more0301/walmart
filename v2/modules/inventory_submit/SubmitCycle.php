...
use WB\helpers\xml\templates\InventoryTemplate;
use WB\Helpers\Xml\XmlRead;

require_once APP_ROOT . '/helpers/RequestParameters.php';
require_once APP_ROOT . '/helpers/Guid.php';
require_once APP_ROOT . '/helpers/xml/templates/InventoryTemplate.php';
require_once APP_ROOT . '/helpers/xml/XmlRead.php';

class SubmitCycle extends InventorySubmitModule
{
    public function cycle(array $data): int
    {
        $parts = array_chunk(
            (array)$data,
            App::$options['shops'][App::$shopId]['max_inventory_submit']
        );
        ...
        foreach ($parts as $key => $part) {
            ...
            $xml_feed = InventoryTemplate::getFeed($part);
    ...
    public function addFeedToDb(string $feed, int $total_items)
    {
        App::$db->run(
            'INSERT INTO walmart_ca.inventory_feed 
